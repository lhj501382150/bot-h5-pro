...
public class MybatisPlusPageHelper {
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public static PageResult findPage(PageRequest pageRequest, Object mapper, String queryMethodName,QueryWrapper qw) {
		// 设置分页参数
		Page<?> page = getPage(pageRequest);
		Object[] args = new Object[]{page,qw};
		// 利用反射调用查询方法
	 ...
	 * @return
	 */
	public static PageResult getPageResult(PageRequest pageRequest, IPage<?> pageInfo) {
	...
	public static QueryWrapper<?> getQueryWrapper(PageRequest pageRequest){
		...
		for(Entry entry:params.entrySet()){
			...
			if(key.contains("@")){
				...
				}else if("IN".equals(temp)){
					qw.in(paras[0], (List)val);
				}else {
					qw.eq(paras[0], val);
				}
			}else{
...
//		是否判断数据权限
		if(!StringUtils.isBlank(pageRequest.getOwner()) && !SecurityUtils.getUsername().equals(SysConstants.ADMIN)) {
			qw.eq("OWNER", pageRequest.getOwner());
		}
