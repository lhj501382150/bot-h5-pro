 ...
 * @since 2021-04-14
 */
public interface HisUsermoneyMapper extends BaseMapper<HisUsermoney> {
	...
	Page<HisUsermoney> findPageByUser(Page<HisUsermoney> page,@Param(Constants.WRAPPER)Wrapper wrapper);
	@Select("select sum(start) as start,sum(inmoney) as inmoney,sum(outmoney) as outmoney,sum(income) as income,sum(payment) as payment,sum(`in`) as `in`,sum(`out`) as `out` "
			...
			+ "t2.uno1,t2.uno2,t2.uno3,t2.uno4,t2.uno5,t2.uno6,t2.uno7,t2.uno8,t2.uno9,t2.uno10,t2.uno11,t2.uno12,t2.uno13,t2.uno14 "
			+ " from tb_zj_his_usermoney t1 left join TB_HY_USER_RELATION t2 on t1.USERNO = t2.USERNO"
			+ " left join tb_hy_user t4 on t1.userno = t4.userno "
			+ " left join tb_zj_account t3 on t1.acctno = t3.acctno "
			+ " left join tb_hy_user t5 on t1.userno = t5.userno "
			+ ") t ${ew.customSqlSegment} order by fdate desc ")
	HisUsermoney findSum(@Param(Constants.WRAPPER)Wrapper wrapper);
	...
	@Select("select t.* from ("
			...
			+ " from (                                                                                                "
			+ " 	select left(fdate,7) as fdate,userno,acctno, start from tb_zj_his_usermoney                           "
			+ " 	where (fdate,userno,acctno) in (                                                                  "
			+ " 		select min(fdate),userno,acctno from tb_zj_his_usermoney group by left(fdate,7),userno,acctno     "
			+ " 	)) t1                                                                                             "
			+ " left join (                                                                                           "
			+ " 	select left(fdate,7) as fdate,userno,acctno, freeze,balance,enable from tb_zj_his_usermoney                  "
			+ " 	where (fdate,userno,acctno) in (                                                                  "
			+ " 		select max(fdate),userno,acctno from tb_zj_his_usermoney group by left(fdate,7),userno,acctno     "
			...
			+ " 	,sum(`out`) as payment                                                                            "
			+ " 	from (                                                                                            "
			+ " 	select left(fdate,7) as fmonth, t1.* from tb_zj_his_usermoney t1                                      "
			...
			+ ") t ${ew.customSqlSegment} order by fdate desc")
	Page<HisUsermoney> findMonthPage(Page<HisUsermoney> page,@Param(Constants.WRAPPER)Wrapper wrapper);
