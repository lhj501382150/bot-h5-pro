<template>
	<view class="order">
		<uni-nav-bar left-icon="left"  title="历史投注记录" background-color="rgb(40,148,255)" color="#fff" :border="false" @clickLeft="goBack"></uni-nav-bar>
		<uni-datetime-picker type="datetime" placeholder="开始日期" v-model="startDate"/>
		<uni-datetime-picker type="datetime" placeholder="结束日期" v-model="endDate"/>
		<uni-easyinput placeholder="所属账号" v-model="searchUserno"></uni-easyinput>
		<button type="primary" class="query-btn" size="mini" @click="searchData" :loading="isLoading" :disabled="isLoading">查询</button>
		<view class="tab-bar">
	...
	export default {
		...
		methods: {
			...
			loadData(){
				...
				if(this.searchUserno != ''){
					this.search.userno = this.searchUserno
				}
