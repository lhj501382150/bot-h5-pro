<template>
  <div class="page-container">
	...
	<el-dialog :title="operation?'新增':'编辑'" width="40%" :visible.sync="dialogVisible" :close-on-click-modal="false" :destroy-on-close="true" v-dialogDrag>
        ...
        <el-select v-model="dataForm.actno" placeholder="赛事活动" style="width: 100%;" clearable>
      ...
      <el-form-item label="奖励方式" prop="rewtype">
        <el-select v-model="dataForm.rewtype" placeholder="奖励方式" style="width: 100%;" >
          <el-option v-for="(item,index) in rewtypes" :key="index" :label="item.val" :value="item.key"></el-option>
        </el-select>
      </el-form-item>
			<el-form-item label="排名最小号" prop="minno">
				<el-input v-model.number="dataForm.minno" auto-complete="off"></el-input>
			</el-form-item>
			<el-form-item label="排名最大号" prop="manno">
				<el-input v-model.number="dataForm.manno" auto-complete="off"></el-input>
			</el-form-item>
			<el-form-item label="奖励积分（%）" prop="rewmoney">
				<el-input v-model="dataForm.rewmoney" auto-complete="off"></el-input>
			</el-form-item>
		</el-form>
...
export default {
	...
	data() {
        ...
        minno: [
          { required: true, message: '请输入排名最小号', trigger: 'blur' }
        ],
        manno: [
          { required: true, message: '请输入排名最大号', trigger: 'blur' }
        ],
        rewmoney: [
	...
	methods: {
        ...
        this.pageRequest = {pageNum: 1, pageSize: 50}
			...
			this.$api.rewar.findPage(this.pageRequest).then((res) => {
	  ...
	  findAct:function(){
		...
		handleDelete: function (data) {
			this.$api.rewar.batchDelete(data.params).then(data!=null?data.callback:'')
		},
		// 显示新增界面
		handleAdd: function () {
		...
		submitForm: function () {
            ...
            if(this.operation){
              this.$api.rewar.save(params).then((res) => {
              ...
              this.$api.rewar.edit(params).then((res) => {
                this.editLoading = false
                if(res.code == 200) {
                  this.$message({ message: '操作成功', type: 'success' })
                  this.dialogVisible = false
	...
	initColumns: function () {
        ...
        {prop:"rewtype", label:"奖励方式", minWidth:100,formatter:this.rewtypeFormat},
				{prop:"minno", label:"排名最小号", minWidth:100},
				{prop:"manno", label:"排名最大号", minWidth:120},
				{prop:"rewmoney", label:"奖励积分（%）", minWidth:100}
			]
