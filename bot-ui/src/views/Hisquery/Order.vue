      ...
      <el-form-item>
        <el-select v-model="filters.mode" placeholder="类型" clearable>
          ...
          <el-option label="牛牛" value="1"></el-option>
			...
			<el-form-item>
				<kt-button icon="fa fa-search" :label="$t('action.search')" perms="order:his:view" type="primary" @click="findPage(null)"/>
			</el-form-item>
		</el-form>
	</div>
	<!--表格内容栏-->
...
export default {
	...
	methods: {
        ...
        'mode':this.filters.mode
			...
			this.$api.hisorder.findPage(this.pageRequest).then((res) => {
