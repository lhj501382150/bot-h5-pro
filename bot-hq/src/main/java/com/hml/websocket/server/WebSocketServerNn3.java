...
//这个是客户端访问的接口
@Slf4j
@ServerEndpoint("/socket/niu3/{userno}/{md5}")
@Component
public class WebSocketServerNn3 {
	@Autowired
    ...
    private static ConcurrentHashMap<String,WebSocketServerNn3> webSocketMap = new ConcurrentHashMap<>();
    ...
    public void onOpen(Session session, @PathParam("userno") String userno, @PathParam("md5") String md5) {
        ...
        try {
        	DataSource item = HqTaskManager.getDraw("HXNN3",redisUtils);
        	if(item == null) {
        		item =new DataSource();
        	}
...
//        error.printStackTrace();
    ...
    public static synchronized void addOnlineCount() {
        WebSocketServerNn3.onlineCount++;
    }
