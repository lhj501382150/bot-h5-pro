...
//这个是客户端访问的接口
@Slf4j
@ServerEndpoint("/socket/bd5/{userno}/{md5}")
@Component
public class WebSocketServerBd5 {
	@Autowired
    ...
    private static ConcurrentHashMap<String,WebSocketServerBd5> webSocketMap = new ConcurrentHashMap<>();
    ...
    public void onOpen(Session session, @PathParam("userno") String userno, @PathParam("md5") String md5) {
        ...
        try {
        	DataSource item = HqTaskManager.getDraw("HXBD5",redisUtils);
        	if(item == null) {
        		item =new DataSource();
        	}
...
//        error.printStackTrace();
    ...
    public static synchronized void addOnlineCount() {
        WebSocketServerBd5.onlineCount++;
    }
