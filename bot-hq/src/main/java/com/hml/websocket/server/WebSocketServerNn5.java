...
//这个是客户端访问的接口
@Slf4j
@ServerEndpoint("/socket/niu5/{userno}/{md5}")
@Component
public class WebSocketServerNn5 {
	@Autowired
    ...
    private static ConcurrentHashMap<String,WebSocketServerNn5> webSocketMap = new ConcurrentHashMap<>();
    ...
    public void onOpen(Session session, @PathParam("userno") String userno, @PathParam("md5") String md5) {
        ...
        try {
        	DataSource item = HqTaskManager.getDraw("HXNN5",redisUtils);
        	if(item == null) {
        		item =new DataSource();
        	}
...
//        error.printStackTrace();
    ...
    public static synchronized void addOnlineCount() {
        WebSocketServerNn5.onlineCount++;
    }
